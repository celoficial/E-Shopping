...
{
    //TODO: REPLACE WITH OTHER MIGRATION STRATEGY/SOLUTION
    public static class DbExtensions
    {
        public static IHost MigrateDatabase<TContext>(this IHost host)
        {
            using (var scope = host.Services.CreateScope())
            {
                ...
                {
                    logger.LogError("Discount DB Migration ERROR: {Error}", e.InnerException);
                }
            }
        ...
        {
            using var connection = new Npgsql.NpgsqlConnection(config.GetValue<string>("DatabaseSettings:ConnectionString"));
            connection.Open();
